...
class LCMOrchestratorV2:
    ...
    内容编码层集成：
    - LCM 核心不感知具体编码实现
    - 通过 ContentEncodingRegistry 发现和调用编码器
    - 编码器在 system_prompt 和 response 两个钩子点介入
    """
    ...
    def __init__(
        ...
        self._content_encoding = content_encoding or self._resolve_encoding(encoding_type)
    ...
    def _resolve_encoding(self, encoding_type: EncodingType) -> ContentEncoding:
        """从注册表解析编码器

        LCM 核心不感知具体编码实现，通过注册表查找。
        找不到时回退到恒等编码（无变换）。
        """
        registry = get_default_registry()
        encoder = registry.get(encoding_type)
        if encoder.encoding_type == EncodingType.IDENTITY and encoding_type != EncodingType.IDENTITY:
            logger.warning(
                "请求的编码未注册，回退到恒等编码",
                requested=encoding_type.value,
            )
        return encoder

    def _build_encoding_context(self) -> EncodingContext:
        """构建编码上下文"""
    ...
    def _encode_system_prompt(self, system_prompt: str) -> str:
        """编码系统提示词（内容编码层钩子）"""
        if not self._content_encoding:
            return system_prompt
        try:
            context = self._build_encoding_context()
            return self._content_encoding.encode_system_prompt(system_prompt, context)
        except Exception as e:
            logger.warning("系统提示词编码失败，使用原始提示词", error=str(e))
            return system_prompt

    def _encode_response(self, response_text: str) -> str:
        """编码模型响应（内容编码层钩子）"""
        if not self._content_encoding:
            return response_text
        try:
            context = self._build_encoding_context()
            return self._content_encoding.encode_response(response_text, context)
        except Exception as e:
            logger.warning("响应编码失败，使用原始响应", error=str(e))
            return response_text
    ...
    def run_stream(
        ...
        # --- 内容编码层钩子：编码 system prompt ---
        if current_messages and current_messages[0]["role"] == "system":
            original_sp = current_messages[0]["content"]
            encoded_sp = self._encode_system_prompt(original_sp)
            current_messages[0]["content"] = encoded_sp
            if encoded_sp != original_sp:
                self._emit("encoding_applied", metadata={
                    "encoding": self._content_encoding.encoding_type.value if self._content_encoding else "none",
                    "original_length": len(original_sp),
                    "encoded_length": len(encoded_sp),
                })
        ...
        while self._round < self.max_rounds:
            ...
            try:
                ...
                encoded_error = self._encode_response(self.detector.get_clean_buffer())
                yield encoded_error
                return
            ...
            if not requests:
                ...
                # --- 内容编码层钩子：编码响应 ---
                clean = self._encode_response(clean)
        ...
        final_buffer = self._encode_response(self.detector.get_clean_buffer())
        yield final_buffer
